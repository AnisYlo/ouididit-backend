...
router.post("/", (req, res) => {
  ...
  newChat.save().then((data) => {
    if (data !== null) {
      res.json({ result: true, chatId: data._id });
...
router.post("/byActivities", async (req, res) => {
  ...
  if (!Array.isArray(activityIds) || !activityIds.length === 24) {
    return res
      .status(400)
      .json({ result: false, error: "Invalid activity IDs" });
  }
...
router.get("/:chatId", (req, res) => {
  if (req.params.chatId.length !== 24) {
...
router.get("/:chatId/:userToken", (req, res) => {
  ...
  Chat.findById(req.params.chatId)
    ...
    .then((chat) => {
      ...
      const lastLogoff = chat.messages.filter(
        (msg) => msg.type === "Event" && msg.user.token === req.params.userToken
      );

      let newMessagesCount = 0;
      // If user never logoff from the chat all messages are new
      if (lastLogoff.length === 0) {
        newMessagesCount = chat.messages.filter(
        ...
        newMessagesCount = chat.messages.filter(
          (msg) =>
            msg.type === "Message" && msg.createdAt > lastLogoff[0].createdAt
        ).length;
      }
  ...
  Chat.findOne({ _id: req.params.chatId })
    ...
    .then((data) => {
      if (data !== null) {
        ...
        res.status(404).json({ result: false, error: "Activity not found" });
    })
    .then((tempActivityId) => {
      // Save activity ID
      ...
      User.findOne({ token: req.params.userToken })
        ...
        .then((tempUser) => {
          // Save user ID
          user = tempUser;
          Participant.findOne({ user: user._id, activity: activityId })
            .select("_id")
            .then((data) => {
              if (data !== null) {
                // Connect user to PUSHER
                console.log("User join =>", user.username);
                // pusher.trigger(chatName, event, user)
                pusher.trigger(req.params.chatId, "join", {
                ...
                res.status(401).json({
                  result: false,
                  error: "User unauthorized to join this chanel",
                });
              }
            });
        });
  ...
  User.findOne({ token: req.params.userToken })
    ...
    .then((data) => {
      if (data !== null) {
        ...
        res
          .status(500)
          .json({ result: false, error: "Error during finding user" });
    })
    .then((tempUser) => {
      // Save user ID
      user = tempUser;
        ...
        .then(() => {
          ...
          Chat.findOneAndUpdate(
            ...
            .then(() => {
              ...
              // pusher.trigger(chatName, event, user)
              pusher.trigger(req.params.chatId, "leave", {
                username: user.username,
              });
              res.json({ result: true });
            });
...
router.delete("/:chatId", (req, res) => {
  ...
  Chat.findByIdAndDelete(req.params.chatId).then((data) => {
    if (data) {
